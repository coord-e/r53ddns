...
use rusoto_route53::{
    ...
    GetChangeRequest, GetChangeResponse, ResourceRecord, ResourceRecordSet, Route53, Route53Client,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let value = if let Some(ip) = opt.ip {
        ip
    } else {
        get_ip().await?
    };
    ...
    let client = if let (Some(key), Some(secret)) = (opt.key, opt.secret) {
